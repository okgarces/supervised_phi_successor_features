...
class ReplayMemory:
    ...
    def update_sequence_priority(self, index, update_pre_next_seq_priority=False):
        self.memory['sequence_priority'][index] = 1
...
class DQNAgentConfig:
    ...
    n_step_q_learning: int = 3  # 5 Default in Carvalho2023
    ...
    trace_length: int = 1
    overlap_length: int = 1  # Default according R2D2 paper
    ...
    min_learning_rate_factor: float = 1
    total_iters_learning_rate: int = 1_000_000 # the number of sgd steps
    gamma: float = 0.99
...
class DQN_Model(torch.nn.Module):
    ...
    def set_recurrent_hidden_state(self, hs, cs):
        pass
    ...
    def forward(self, obs, action_taken, mode='rollout'):
        ...
        vision_output = self._vision_net(vision_input).view(n_batch, -1)  # [n_batch, n_trace, dim] restore trace dim

        q_values = self._dqn_model(vision_output).view(n_batch, self.dqn_config.n_actions)
...
class R2D1_NStep:
    ...
    def __init__(self, source_tasks, target_tasks, vector_tasks, config):
        ...
        self.number_of_sgd_steps = 1

        # Debug purposes
        self.debug_gradients = True
        self.current_state_from_render = True
    ...
    def process_current_state(self, obs, env):
        ...
        if self.current_state_from_render:
            ...
            current_obs = {'image': cv2.resize(env.render(), [80, 80])}  # For CARTPOLE
    ...
    def set_seq_start_index(self):
        ...
        stop_index = max(last_index - self.config.trace_length, last_index)

        seq_start_index = [i for i in range(start_index, stop_index)]
        ...
        for source_task_id, env in enumerate(self.source_tasks):
            ...
            for training_step in range(self.n_training_steps):
                ...
                with torch.no_grad():
                    ...
                    current_q_value, *_ = self.policy_network(current_state, previous_action)
                    target_q_value, *_ = self.target_network(current_state, previous_action)
                ...
                if terminated or truncated: #training_step % self.config.log_performance_n_training_steps == 0:
                ...
                if (training_step > self.minimum_exploration_steps) and (training_step % 10) == 0:
                    ...
                    if replay_buffer is not None:
                        ...
                        with torch.no_grad():
                            ...
                            next_actions = torch.argmax(target_q_value_policy_network , axis=1, keepdim=True)  # GPI to get actions Double DQN
                            max_target_q_value = target_q_value.gather(1, next_actions)
                            target_q_values = batch_rewards + (gammas * (1 - batch_terminated_masks)) * max_target_q_value.view(self.n_batch, -1)  # Remove last dimension no needed
                        ...
                        # current q value
                        batch_actions = to_tensor(batch_actions).view(self.n_batch, -1) # Same dimensions in the samples.
                        current_q_value = current_q_value.gather(1, batch_actions).view(self.n_batch, -1)

                        # First loss Q-Loss
                        # q_loss_n_trace = 0.5 * torch.sum(torch.square(target_q_values.view(self.n_batch, -1) - current_q_value), dim=0)  # [B, ]
                        # q_loss = torch.mean(q_loss_n_trace, dim=0)  # mean over traces

                        q_loss = self.loss(target_q_values.view(self.n_batch, -1), current_q_value)
...
if __name__ == '__main__':
    ...
    msf = R2D1_NStep(training_envs, testing_envs, [], config)
    msf.execute()
