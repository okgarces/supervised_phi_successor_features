            ...
            for training_step in range(self.n_training_steps):
                ...
                with torch.no_grad():
                    ...
                    if (random.random() <= self.epsilon):
                        ...
                        max_q_value_sf = torch.max(current_q_value_sf,
                                                   axis=1).values  # [n_batch, d_z_samples, n_actions ]
                ...
                if (training_step > minimum_exploration_steps) and (training_step % 10) == 0:
                    ...
                    if replay_buffer is not None:
                        ...
                        with torch.no_grad():
                            ...
                            next_actions = torch.argmax(max_target_q_value_sf, axis=3,
                                                        keepdim=True)  # GPI to get actions
                            max_target_q_value_sf = max_target_q_value_sf.gather(3, next_actions)

                            target_q_values = batch_rewards + (
                                        gammas * (1 - batch_terminated_masks)) * max_target_q_value_sf.view(
                                self.config.trace_length, self.n_batch, -1)  # Remove last dimension no needed

                            # torch.Size([40, 32, 30, 4, 4])
                            # Compute target successor features
                            # [n_trace, n_batch, dz_samples, n_actions, d_features_dim]
                            # max_q_values_from_z_samples = max_target_q_values_indices.indices.unsqueeze(-1).repeat(1, 1,
                            #                                                                                        1, 1,
                                                                                                                   # 4)
                            # target_sf = target_sf.gather(2,
                            #                              max_q_values_from_z_samples)  # target_sf from best q_values samples

                            # next_actions_sf = next_actions.unsqueeze(-1).repeat(1, 1, 1, 1, 4)
                            # target_sf = target_sf.gather(3,
                            #                              next_actions_sf)  # .reshape(target_sf.shape[0], target_sf.shape[1], -1) # reshape [n_batch, d_features]

                            # target_sf_values = batch_phis + (
                            #             gammas * (1 - batch_terminated_masks) * target_sf.view(self.config.trace_length,
                            #                                                                    self.n_batch,
                            #                                                                    -1))  # Remember the dimension
                        ...
                        # current q value
                        batch_actions = to_tensor(batch_actions).to(dtype=torch.int64).unsqueeze(2).repeat(1, 1, 1,
                                                                                                           1)  # Same dimensions in the samples. In this case 30 is the number of z samples.
                        ...
                        priority = (np.abs((current_q_value_sf - target_q_values).detach().cpu().numpy()).reshape(
