...
class DQNAgentConfig:
    epsilon: int = 1.0 # 0.99 # Borsa2020 keeps 0.1 fixed
                ...
                if (training_step > minimum_exploration_steps) and (training_step % 10) == 0:
                    ...
                    if replay_buffer is not None:
                        ...
                        with torch.no_grad():
                            ...
                            target_q_values = batch_rewards + (gammas) * max_target_q_value_sf.view(self.config.trace_length, self.n_batch, -1)  # Remove last dimension no needed

                            if self.config.use_full_loss or self.config.use_psi_loss: # Use full loss psi + phi + q_losses (Auxiliary tasks)
                                ...
                                target_sf_values = batch_phis.unsqueeze(-2).unsqueeze(-2).repeat(1,1,self.sf_config.d_z_samples + 1,1,1) + (gammas) * max_target_sf_value
                        ...
                        if self.config.use_full_loss:
                            ...
                            psi_loss = torch.mean(0.5 * torch.sum(torch.square(target_sf_values - current_sf), dim=0))
                        ...
                        elif self.config.use_psi_loss:
                            current_actions_sf = batch_actions.unsqueeze(-1).repeat(1, 1, 1, 1,
                                                                                    self.sf_config.features_dim)  # Add features dimension in actions.
                            current_sf = current_sf.gather(3, current_actions_sf)
                            psi_loss = torch.mean(0.5 * torch.sum(torch.square(target_sf_values - current_sf), dim=0))
                            accum_loss = psi_loss
                        else:
