...
class DQN_USFA_Model(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self._sf_model = torch.nn.Sequential(
            torch.nn.Linear(self.sf_config.states_dim +  self.sf_config.d_z_samples * self.sf_config.features_dim, 128 * 2),
            torch.nn.ReLU(),
            torch.nn.Linear(128 * 2, 512),
            torch.nn.ReLU(),
            torch.nn.Linear(512, 512),
            torch.nn.ReLU(),
            torch.nn.Linear(512, self.sf_config.d_z_samples * self.sf_config.features_dim * self.sf_config.n_actions)
        )
    ...
    def forward(self, obs, reward_mapper, action_taken, mode='rollout'):
        ...
        with torch.no_grad():
            ...
            z_samples = z_samples.view(n_batch, n_trace, -1) 
            ...
            for training_step in range(self.n_training_steps):
                ...
                if training_step % 1 == 0:
                    ...
                    if replay_buffer is not None:
                        ...
                        current_sf, current_q_value_sf, *_ = self.policy_network(batch_states, env.vector_to_reward, batch_actions, 'train')
