...
class ReplayMemory:
    def __init__(self, memory_size=100_000, batch_size=32, n_step=3, state_size=(3,80,80), cell_size=256, action_repeat=1, n_stacks=1, alpha=0.4):
        ...
        self.memory['action'] = np.ones((self.memory_size, 1), dtype=np.int8) * 100 # 100 is a No-Op action
    ...
    def sample(self, device='cpu'):
        ...
        batch['prev_action'] = [[[100]] * len(seq_index)] + [self.memory['action'][(seq_index - 1 + self.burn_in_length + s) % self.memory_size] for s in range(1, self.learning_length)]
        ...
        batch['phi'] = [self.memory['phi'][(seq_index+self.burn_in_length+s)%self.memory_size] for s in range(self.learning_length)]
...
class DQN_USFA_Model(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self._recurrent_net = torch.nn.LSTM(self.sf_config.states_dim + self.sf_config.actions_dim, batch_first=False, hidden_size=self.recurrent_hidden_state_dim)
    ...
    def forward(self, obs, reward_mapper, action_taken, mode='rollout'):
        ...
        if mode == 'train':
            if self.recurrent_cell_state_train is None:
                self.recurrent_hidden_state_train = torch.zeros((1, n_batch, self.recurrent_hidden_state_dim)).to(device) # This is the same dim as hidden in LSTM
                self.recurrent_cell_state_train = torch.zeros((1, n_batch, self.recurrent_hidden_state_dim)).to(device) # This is the same dim as hidden in LSTM
            ...
            if self.recurrent_cell_state_rollout is None:
                self.recurrent_hidden_state_rollout = torch.zeros((1, n_batch, self.recurrent_hidden_state_dim)).to(device) # This is the same dim as hidden in LSTM
                self.recurrent_cell_state_rollout = torch.zeros((1, n_batch, self.recurrent_hidden_state_dim)).to(device) # This is the same dim as hidden in LSTM
        ...
        sf_values = self._sf_model(inputs_to_sf).view(n_trace, n_batch, self.sf_config.d_z_samples, self.sf_config.n_actions, self.sf_config.features_dim)
...
class MSFA_SF_NStep:
    ...
    def __init__(self, source_tasks, target_tasks, vector_tasks, config):
        ...
        if config.n_step_q_learning > 0:
            ...
            self.replay_buffer = ReplayMemory(config.n_replay_samples, config.batch_size, config.n_step_q_learning, action_repeat=1)
    ...
    def set_seq_start_index(self):
        ...
        seq_start_index = [i for i in range(start_index, last_index - self.config.trace_length, self.config.overlap_length)]
        ...
        for source_task_id, env in enumerate(self.source_tasks):
            ...
            for training_step in range(self.n_training_steps):
                ...
                with torch.no_grad():
                    ...
                    current_sf, current_q_value_sf, hidden_state, cell_state = self.policy_network(current_state, env.vector_to_reward, previous_action)
                    target_sf, target_q_value_sf, target_hidden_state, target_cell_state = self.target_network(current_state, env.vector_to_reward, previous_action)
                ...
                if (training_step > minimum_exploration_steps) and (training_step % 10) == 0:
                    ...
                    if replay_buffer is not None:
                        ...
                        with torch.no_grad():
                            ...
                            target_sf, target_q_value_sf, *_ = self.target_network(batch_next_states, env.vector_to_reward, batch_actions, 'train') # USFA needs states and actions?
                            ...
                            next_actions = torch.argmax(max_target_q_value_sf, axis=3, keepdim=True) # GPI to get actions
                            ...
                            # torch.Size([40, 32, 30, 4, 4]) 
                            # Compute target successor features
                            # [n_trace, n_batch, dz_samples, n_actions, d_features_dim]
                            max_q_values_from_z_samples = max_target_q_values_indices.indices.unsqueeze(-1).repeat(1,1,1,1,4)
                            target_sf = target_sf.gather(2, max_q_values_from_z_samples) # target_sf from best q_values samples

                            next_actions_sf = next_actions.unsqueeze(-1).repeat(1,1,1,1,4)
                            target_sf = target_sf.gather(3, next_actions_sf) #.reshape(target_sf.shape[0], target_sf.shape[1], -1) # reshape [n_batch, d_features]
                                                        
                            target_sf_values = batch_phis + (gammas * (1 - batch_terminated_masks) * target_sf.view(self.config.trace_length, self.n_batch, -1)) # Remember the dimension 
                        
                        current_sf, current_q_value_sf, *_ = self.policy_network(batch_states, env.vector_to_reward, batch_prev_actions, 'train')
                        
                        #  [n_trace, n_batch, d_z_sample, n_actions, d_features]
                        # current q value
                        batch_actions = to_tensor(batch_actions).to(dtype=torch.int64).unsqueeze(2).repeat(1,1,1,1) # Same dimensions in the samples. In this case 30 is the number of z samples.
                        max_current_q_values_indices = current_q_value_sf.max(axis=2, keepdim=True)
                        
                        current_q_value_sf = max_current_q_values_indices.values.gather(3, batch_actions)
                        current_q_value_sf = current_q_value_sf.reshape(current_q_value_sf.shape[0], current_q_value_sf.shape[1], -1) # keep [n_trace, n_batch, 1]
                        
                        current_actions_sf = batch_actions.unsqueeze(-1).repeat(1,1,1,1,4) # Add features dimension in actions.
                        max_q_values_from_z_samples = max_current_q_values_indices.indices.unsqueeze(-1).repeat(1,1,1,1,4)
                        ...
                        priority = (np.abs((current_q_value_sf - target_q_values).detach().cpu().numpy()).reshape(-1) + self.priority_epsilon) ** self.priority_alpha
