...
class DQNAgentConfig:
    epsilon: int = 0.5 # 0.99 # Borsa2020 keeps 0.1 fixed
    ...
    trace_length: int = 10
    overlap_length: int = 5  # Default according R2D2 paper
...
class DQNConfig:
    ...
    states_dim: int = 4  # This is the same as the VisionConfig output_layers_dim
...
class DQN_Model(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self._dqn_model = torch.nn.Sequential(
            torch.nn.Linear(self.recurrent_hidden_state_dim, 128),
            torch.nn.ReLU(),
            torch.nn.Linear(128, 64),
            torch.nn.ReLU(),
            torch.nn.Linear(64, 32),
            torch.nn.ReLU(),
            torch.nn.Linear(32, self.dqn_config.n_actions)
        )
...
class R2D1_NStep:
    ...
    def __init__(self, source_tasks, target_tasks, vector_tasks, config):
        ...
        if config.n_step_q_learning > 0:
            ...
            self.replay_buffer = ReplayMemory(config.n_replay_samples, config.batch_size, config.n_step_q_learning, state_size=(4,),
                                              action_repeat=1)
